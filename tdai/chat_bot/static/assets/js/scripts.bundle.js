...
var KTChat = function () {
	var initChat = function (parentEl) {
		...
		// messaging
		var handleMessaging = function(response) {
            var answers = [];
            ...
            if (response.hasOwnProperty('step_id')) {
                step_id = response.step_id;
            }

            if (answers.length > 0 && answers[answers.length - 1].card_type === 'form') {
                check_form = true;
            } else {
                check_form = false;
            }
            ...
            if (textarea.value.length === 0 && answers.length === 0) {
                return;
            }
            ...
            <div class="kt-chat__user">
                ...
                <a href="#" class="kt-chat__username">Người Dùng</span></a>
                <span class="kt-media kt-media--circle kt-media--sm">
                    <img src="https://lh3.googleusercontent.com/G36XEk4qJCxIPX0pl0A-6WmzArz3sSMWRjQdjBJYaVz6s0u_uLvOu1u7a1Tta-H1BRyP">
                </span>
            </div>
            ...
            for (var i = 0; i < answers.length; i++) {
                var node = document.createElement("DIV");
                    ...
                    var html_buttons = ''; // buttons
                    for (var j = 0; j < answers[i]?.buttons?.length; j++) {
                        html_buttons += `<button type="button" class="btn btn-primary kt-chat__directional mt-2" id="${answers[i].buttons[j].step_id}">${answers[i].buttons[j].step_name}</button>`
                    }
                    var html_bot = `
                    <div class="kt-chat__user">
                        <span class="kt-media kt-media--circle kt-media--sm">
                            <img src="https://i.ibb.co/sgnrb4L/tandan-logo.jpg">
                        </span>
                        <a href="#" class="kt-chat__username">Chat BOT</span></a>
                        <span class="kt-chat__datetime">Just now</span>
                    </div>
        ...
        // click button reply
		KTUtil.on(parentEl, '.kt-chat__reply', 'click', function(e) {
            var sentence = $("#chat_input").val();
            if (check_form) {
                $.ajax({
                    url : "/api/qa",
        ...
        // Click button
		KTUtil.on(parentEl, '.kt-chat__directional', 'click', function(e) {
            var sentence =  $(this).text();
            var step_id = $(this).attr('id');
            $.ajax({
                url : "/api/steps/jumpto",
                type: "POST",
                data : JSON.stringify({
                    user_id: 2,
                    step_id
